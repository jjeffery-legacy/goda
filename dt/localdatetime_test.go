...
func BenchmarkDateTime(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		minute := 2
		second := 31
		_ = DateTime(year, month, day, hour, minute, second)
	}
}
...
func TestDateTimeYears(t *testing.T) {
	for year := -9999; year <= 9999; year++ {
		...
		datetime := DateTime(year, time.Month(month), day, hour, minute, second)
...
func CheckLocalDateTime(t *testing.T, datetime LocalDateTime, year, month, day, hour, minute, second int) {
	assert := assert.New(t)
...
func TestDateTimeMarshalXML(t *testing.T) {
	...
	type testStruct struct {
		...
		AnotherElement LocalDateTime `xml:"another"`
		Attribute1     LocalDateTime `xml:",attr"`
		Attribute2     LocalDateTime `xml:"attribute-2,attr"`
	}
